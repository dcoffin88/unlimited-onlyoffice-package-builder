...
```
cd ~/onlyoffice_repos
git clone git@github.com:dcoffin88/web-apps.git
```
...
We just replace the hyphen with a dot. 9.0.2-1 is now: 9.0.2.1.
...
sed -i 's/unlimited_tag_suffix = "-btactic"/unlimited_tag_suffix = "-dcoffin88"/g' scripts/base.py
```
.
...
```
sudo usermod -a -G docker dcoffin88
```

### Docker user - Re-login

In order to be able to use Docker properly from `dcoffin88` user you might need to logout and then login to your user.
...
Also make sure to run the usual 'Hello world' docker examples under the `dcoffin88` user.
...
*Note: The commands below need to be run as the `dcoffin88` user.*

You need to run the command below in order to create a key.

```
ssh-keygen -t rsa -b 4096 -C "dcoffin88@domain.com"
```
...
./onlyoffice-package-builder.sh --product-version=9.0.2 --build-number=1 --unlimited-organization=dcoffin88 --tag-suffix=-dcoffin88 --debian-package-suffix=-dcoffin88
```

### Final deb package

The final `onlyoffice-documentserver_9.0.2-1-dcoffin88_amd64.deb` deb package can be found at: `~/build-oo/unlimited-onlyoffice-package-builder/document-server-package/deb/` directory.
...
git tag -a 'builds-debian-11/9.0.2.1' -m 'builds-debian-11/9.0.2.1'
git push origin 'builds-debian-11/9.0.2.1'
```
.
